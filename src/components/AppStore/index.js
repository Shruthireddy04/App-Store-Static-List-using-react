...
class AppStore extends Component {
  ...
  getFilteredList = apps => {
    const {activeTabId, searchInput} = this.state
    const filteredList = apps.filter(
      i =>
        i.category === activeTabId &&
        i.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return filteredList
  }
  ...
  render() {
    ...
    const filteredList = this.getFilteredList(appsList)
          ...
          <ul className="tabItem-cnt">
            {tabsList.map(i => (
              <TabItem
                ...
                key={i.tabId}
                id={i.tabId}
                isActive={activeTabId === i.tabId}
              />
            ))}
          </ul>
          <ul className="appItem-cnt">
            {filteredList.map(i => (
              <AppItem key={i.appId} appItemDetails={i} />
            ))}
          </ul>
        </div>
      </div>
